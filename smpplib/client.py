...
import struct
import warnings
#from airtel_trx_client import reconnect
...
class Client(object):
    ...
    def __init__(
        ...
        self.last_resp_connection=current_time_in_india.strftime('%Y-%m-%d %H:%M:%S')
        ...
        while True:
            try:
                ...
                try:
                    ...
                    self.connect()
                    if(self._socket is not None):
                        self.logger.info(self.mode+" id:"+str(self.system_id)+" password:"+str(self.password))
                        if(self.mode=="TRX"):
                            self._bind('bind_transceiver', system_id=self.system_id, password=self.password)
                        if(self.mode=="TX"):
                            india_timezone = pytz.timezone("Asia/Kolkata")
                            current_time_in_india = datetime.now(india_timezone)
                            self.last_resp_connection=current_time_in_india.strftime('%Y-%m-%d %H:%M:%S')
                            self._bind('bind_transmitter', system_id=self.system_id, password=self.password)
                            
                        if(self.mode=="RX"):
                            india_timezone = pytz.timezone("Asia/Kolkata")
                            current_time_in_india = datetime.now(india_timezone)
                            self.last_resp_connection=current_time_in_india.strftime('%Y-%m-%d %H:%M:%S')
                            self._bind('bind_receiver', system_id=self.system_id, password=self.password)
                except Exception as e:
