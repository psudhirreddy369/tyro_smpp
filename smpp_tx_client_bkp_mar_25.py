...
def send_message(msg_id: str, source: str, destination: str, message_type: str, message: str, template_id: str, pe_id: str, telemarketing_id: str,encoding_flag:int,msg_type_flag:int,msg_parts:int,msg_part:int,unique_ref:int):
    ...
    submitdict = {'msgId': msg_id, 'subTime': timestamp, 'smscId': '',
                    ...
                    'msgcount': 1, 'msg_part_num': msg_part,'msgtype': message_type}
...
def read_messages_from_kafka_queue():
    ...
    for msg in consumer:
        ...
        try:
            ...
            send_message(message['messageid'], message['sender'], message['dest'], message['messagetype'], message['message'], message['templateid'], message['peid'], TELEMARKETING_ID,message['encoding_flag'],message['msg_type_flag'],message['msg_parts'],message['msg_part'],message['unique_ref'])
            consumer.commit();             
            sleep(0.1)
        except Exception as exception:
            print('Exception :%s',str(exception))
