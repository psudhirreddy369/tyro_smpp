...
from utility import create_sms_submitted,check_msgid_exists
import pandas as pd
import pytz
import smpplib.gsm
...
IS_PREPAID=os.getenv('IS_PREPAID')
...
def read_messages_from_kafka_queue():
    ...
    for msg in consumer:
        ...
        try:
            ...
            print(client.last_resp_connection)
            india_timezone = pytz.timezone("Asia/Kolkata")
            current_time_in_india = datetime.now(india_timezone)
            print(current_time_in_india.strftime('%Y-%m-%d %H:%M:%S'))
