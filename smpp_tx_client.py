...
SUBMIT_CDR=os.getenv('SUBMIT_CDR')
            ...
            for _, msgs in records.items():
                for msg in msgs:
                    ...
                    try:
                        ...
                        if int(diff_seconds) <= int(os.getenv("SOCKET_TIMEOUT", 5)):
...
def handle_submit_sm_resp(pdu):
    ...
    try:
        ...
        rcvsmssubmit=f"{subsmsdict['msgId']}|{subsmsdict['subTime']}|{subsmsdict['smscId']}|{subsmsdict['smsc']}|{subsmsdict['status']}|{subsmsdict['errcode']}\n"

        print(rcvsmssubmit)
        submitted_report(rcvsmssubmit,SUBMIT_CDR)
        print("*****************************************")
